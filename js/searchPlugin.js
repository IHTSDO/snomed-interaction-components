        ...
        if (t != "" && (t != lastT || forceSearch)) {
            if (t.length < 3) {
                ...
                if (isNumber(t)) {
                    ...
                    // check to see if this is a multi-extension search using the keyword 'all'
                    if (options.edition != "all") {
                        // normal edition search
                        ...
                        overallResult.filters.refsetId = [];
                        overallResult.details.total = 0;
                        var totalMatches = 0;
                        while (options.editionList[iCount]) {
                            var searchUrl = options.serverUrl + "/" + options.editionList[iCount].edition + "/" + options.editionList[iCount].release + "/descriptions?query=" + encodeURIComponent(t) + "&limit=100&searchMode=" + panel.options.searchMode + "&lang=" + panel.options.searchLang + "&statusFilter=" + panel.options.statusSearchFilter + "&skipTo=" + skipTo + "&returnLimit=" + returnLimit;
                            console.log(searchUrl);
                            if (panel.options.semTagFilter != "none") {
                                searchUrl = searchUrl + "&semanticFilter=" + panel.options.semTagFilter;
                            }
                            ...
                            if (panel.options.moduleFilter != 'none') {
                                ...
                                searchUrl = searchUrl + "&moduleFilter=" + options.editionList[iCount].module;
                            }
                            if (panel.options.refsetFilter != 'none') {
                                searchUrl = searchUrl + "&refsetFilter=" + panel.options.refsetFilter;
                            }
                                ...
                                success: function(result) {
                                    ...
                                    if (result.filters && panel.options.manifest) {
                                        ...
                                        $.each(result.filters.module, function(i, field) {
                                            var found = false;
                                            var auxObject = {};
                                            $.each(options.editionList, function(j, module) {
                                                if (i == module.langRefsetId) {
                                                    auxObject.term = module.langRefsetTerm;
                                                    auxObject.value = i;
                                                    auxObject.cant = field;
                                                    found = true;
                                                }
                                    ...
                                    overallResult.matches = overallResult.matches.concat(result.matches);
                                    //  overallResult.filters.semTag = overallResult.filters.semTag.concat(result.filters.semTag);
                                    //  overallResult.filters.lang = overallResult.filters.lang.concat(result.filters.lang);
                                    overallResult.filters.module = overallResult.filters.module.concat(result.filters.module);
                                    //overallResult.filters.refsetId = overallResult.filters.refsetId.concat(result.filters.refsetId);
                                    overallResult.details.total = overallResult.details.total + result.details.total;
                        ...
                        //                        result.matches = matchedDescriptions;
                        //result.details.total = totalMatches;
                        ...
                        if (totalMatches > 0) {
                            var searchComment = "<span class='text-muted'>" + totalMatches + " matches found in " + elapsed + " seconds.</span>";
                        }
                        xhr = null;
                        //console.log(JSON.stringify(result));
                        var remaining = totalMatches - (skipTo + returnLimit);

                        if (panel.options.searchMode == "regex") {
                            overallResult.matchedDescriptions.sort(function(a, b) {
                                if (a.term.length < b.term.length)
                                    return -1;
                                if (a.term.length > b.term.length)
                                    return 1;
